...
RunIFS2D = function(transformation, translation, probability, iterations, pixels, set_palette = 0, filename = "")
{
  ...
  if(width == 1 | height == 1) return("Error")
  par(mar=c(0, 0, 0, 0))
  if(set_palette == 0) palette("default")
  if(length(table(result$ImageMatrix)) > 1)
  {
  ...
  if(length(table(result$ImageMatrix)) == 1) palette("default")
  if(set_palette == 0 || length(table(result$ImageMatrix)) == 1)
  {
